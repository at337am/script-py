...
功能概述:
该脚本用于递归地遍历指定目录及其子目录中的所有文件，根据匹配到的文件名模式，将文件名中的时间戳格式化为特定的形式。脚本适用于以特定数字和标识符命名的文件，如 '_ir' 或 '_vis' 类型的图像文件。
...
匹配规则:
- 文件名应包含前10位数字、后续数字、类型标识（'ir' 或 'vis'），以及扩展名（'png' 或 'jpg'）。

注意事项:
1. 该脚本仅重命名文件，不对文件内容做任何更改。
2. 原文件名中的时间戳部分将被重新格式化。
"""
...
def rename_files_in_directory(directory_path):
    ...
    pattern = re.compile(r'(\d{10})(\d+)_(ir|vis)\.(png|jpg)')
